...
if(!empty($_GET['brand']))
    {
        ...
        $products->setBrand_id($_GET['brand']);
        $resultProduct=$products->getProductByBrandID();
        if($resultProduct->num_rows >=1)
        {
            $finalProducts=$resultProduct->fetch_all(MYSQLI_ASSOC);
    ...
    if(!empty($_GET['sub']))
    {
        ...
        $products = new Product;
        $products->setSubcategory_id($_GET['sub']);
        $resultProduct=$products->getProductBySub();
        if($resultProduct->num_rows >=1)
        {
            $finalProducts=$resultProduct->fetch_all(MYSQLI_ASSOC);
    ...
    if(!empty($_GET['cat']))
    {
        ...
        $products = new Product;
        $products->setSubcategory_id($_GET['cat']);
        $resultProduct=$products->getProductByCat();
        if($resultProduct->num_rows >=1)
        {
            $finalProducts=$resultProduct->fetch_all(MYSQLI_ASSOC);
        }
    ...
    if(empty($_GET['brand']) && empty($_GET['sub']) && empty($_GET['cat']))
    {
        $products = new Product;
        $resultProduct=$products->getAllProduct();
        if($resultProduct->num_rows >=1)
        {
            $finalProducts=$resultProduct->fetch_all(MYSQLI_ASSOC);
        }
                            ...
                            <div class="product-width col-xl-4 col-lg-4 col-md-4 col-sm-6 col-12 mb-30">
                                <div class="product-wrapper">
                                    ...
                                    <div class="product-list-details">
                                        <h4>
                                            <a href="product-details.php?product=<?= $product['id'] ?>"><?php echo $product['name_en'];?></a>
                                        </h4>
                                        <div class="product-price-wrapper">
                                            <span>$100.00</span>
                                            <span class="product-price-old"><?php echo $product['price'];?> </span>
